  ...
  return (
    ...
    <Dialog open={opened} onOpenChange={() => setOpened(false)}>
      ...
      <TabsList className="grid w-full grid-cols-2">
        ...
        <TabsTrigger value="creator">Creator/Subscription Details</TabsTrigger>
      </TabsList>
        ):null}
      ...
      <TabsContent value="creator">
          ...
          <div>
            <Label className={`${labelCSS}`}>Subscription Period:</Label>
            <p>{data?.creator?.subscription.period ?? "N/A"}</p>
          </div>

          <div>
            <Label className={`${labelCSS}`}>Started On:</Label>
            <p>{localizedDayjs(data?.creator?.subscription?.startedAt).format("MMMM DD, YYYY")}</p>
          </div>

          <div>
            <Label className={`${labelCSS}`}>Ended On:</Label>
            <p>
              {data?.creator?.subscription.endedAt && localizedDayjs(data.creator?.subscription.endedAt).isValid()
                ? localizedDayjs(data?.creator?.subscription.endedAt).format("MMMM DD, YYYY")
                : "N/A"}
            </p>
          </div>
          ...
          <div>
            <Label className={`${labelCSS}`}>Created On:</Label>
            <p>{localizedDayjs(data?.creator?.subscription.createdAt).format("MMMM DD, YYYY")}</p>
          </div>

          <div>
            <Label className={`${labelCSS}`}>Update On:</Label>
            <p>
              {data?.creator?.subscription.updatedAt && localizedDayjs(data.creator?.subscription.updatedAt).isValid()
                ? localizedDayjs(data?.creator?.subscription.updatedAt).format("MMMM DD, YYYY")
                : "N/A"}
            </p>
          </div>
