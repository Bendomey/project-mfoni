	...
	if (mode === 'VIEW') {
		...
		aboutContent = (
			<div className="mt-5">
				...
				></textarea>
				<span className="text-xs text-gray-400">Maximum of 500 characters</span>
			</div>
		)
	}
	...
	return (
		<>
			{data.map((interest, index) => (
				<span
					key={index}
					className="inline-flex items-center gap-x-2 rounded-md bg-gray-50 px-3 py-2 font-medium text-gray-600 ring-1 ring-inset ring-gray-500/10 md:text-xs"
				>
					{interest}
					<button
						onClick={() => handleRemove(index)}
						type="button"
						className="rounded-xs group relative -mr-1 size-5 hover:bg-gray-500/20 md:size-4"
					>
						<span className="sr-only">Remove</span>
						<svg
							viewBox="0 0 14 14"
							className="size-5 stroke-gray-600/50 group-hover:stroke-gray-600/75 md:size-4"
						>
			...
			<form onSubmit={handleAdd} className="space-x-3">
				<input
					...
					className={classNames(
						'mt-1 w-auto rounded-md border-0 px-2 py-2 text-sm text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-sm sm:leading-6 md:py-1',
					)}
				/>
