...
const getCreatorApplications = async (
  ...
  try {
    ...
    const response = await fetchClient<FetchMultipleDataResponse<CreatorApplication>>(
...
) => useQuery({queryKey: [QUERY_KEYS.CREATOR_APPLICATIONS], queryFn: () => getCreatorApplications(query)})
