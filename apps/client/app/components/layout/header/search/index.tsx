...
import { MagnifyingGlassIcon, XMarkIcon, ViewfinderCircleIcon } from '@heroicons/react/24/outline'
...
export const SearchPhotos = ({ isSittingOnADarkBackground }: Props) => {
    ...
    return (
        <div
            className={`relative focus:ring-0 block hover:border border-transparent border-zinc-300  w-full rounded-full  ${isSearchFocused || isSittingOnADarkBackground ? 'bg-white  border ' : 'bg-zinc-100 '}`}
        >
            <div className='flex flex-row items-center'>
                ...
                <div className='flex-grow'>
                    ...
                    >
                        <div className="absolute left-2 p-2 top-full z-10 mt-2 w-[45vw] overflow-hidden rounded-lg bg-white shadow-lg ring-1 ring-gray-900/5">
                            <TextSearch />
                        </div>
                    </Transition>
                </div>
                ...
                <div className="inset-y-0 right-0 flex items-center pr-6">
                    {
                        query.length ? (
                            <Button variant='unstyled' type='button' onClick={() => setQuery('')} externalClassName='border-r border-zinc-400 mr-4 pr-4'>
                                <XMarkIcon className="h-5 w-5 text-gray-600 cursor-pointer" aria-hidden="true" />
                            </Button>
                        ) : null
                    }
                    <Popover className="relative">
                        <Popover.Button className="flex items-center gap-x-1 text-sm font-semibold leading-6 ring-0 outline-none text-gray-900">
                            <ViewfinderCircleIcon className="h-5 w-5 text-gray-600 cursor-pointer" aria-hidden="true" />
                        </Popover.Button>
