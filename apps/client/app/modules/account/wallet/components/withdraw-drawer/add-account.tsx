...
export function AddNewAccount({ navigate }: Props) {
	...
	const isSubmitButtonDisabled =
		!watch('bankCode') || !watch('accountNumber') || isSubmitting
		...
		<form onSubmit={handleSubmit(mutate)} className="m-5 space-y-5">
			<div>
				...
				<p className="mt-1 text-xs text-gray-400">
					We only list banks/mobile operators in Ghana
				</p>
			...
			<div>
				...
				<div className="mt-1 grid grid-cols-1">
					<input
						...
						className="col-start-1 row-start-1 w-full appearance-none rounded-md bg-white py-1.5 pl-3 pr-8 text-base text-gray-900 outline outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline focus:outline-1 focus:-outline-offset-1 focus:outline-blue-600 sm:text-sm/6"
					/>
				</div>
			</div>
			...
			<AccountDetails
				onSubmit={onRecipientCreate}
				bankCode={watch('bankCode')}
				isOpened={verifyModalState.isOpened}
				onClose={verifyModalState.onClose}
				accountDetails={accountDetails}
				isLoading={isCreatingRecipient}
			/>
		</form>
	)
}
		...
		>
			<div>
				...
				<p className="text-sm text-gray-400">
					Verify if the account details are accurate.
				</p>
				<div className="mt-5 flex items-center space-x-3 bg-gray-50 p-5">
					<div>
						{bankCode === 'VOD' ? (
							<img
								alt="telecel logo"
