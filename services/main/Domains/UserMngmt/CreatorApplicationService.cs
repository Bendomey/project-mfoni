...
public class CreatorApplicationService
{
    ...
    {
        var filterByApplicationId = Builders<CreatorApplication>.Filter.Eq(
            creatorApplication => creatorApplication.Id,
        ...
        if (creatorApplication.Status != CreatorApplicationStatus.PENDING)
        {
            throw new HttpRequestException(
                "CreatorApplicationAlready" + creatorApplication.Status.ToString()
            );
        }
        ...
        var user = await _userCollection
            .Find(user => user.Id == creatorApplication.UserId)
            .FirstOrDefaultAsync();
        if (user is null)
        {
        ...
        {
            var _ = SmsConfiguration.SendSms(
                new SendSmsInput
                {
                    PhoneNumber = user.PhoneNumber,
                    Message = EmailTemplates.CreatorApplicationSubmittedBody.Replace(
                        "{name}",
                        user.Name
                    ),
    ...
    public async Task<Creator> ApproveCreatorApplication(
        ApproveCreatorApplicationInput input,
        string adminId
    )
    {
        ...
        var creatorApplication = await _creatorApplicationCollection
            .Find(application => application.Id == input.CreatorApplicationId)
            .FirstOrDefaultAsync();
        if (creatorApplication is null)
        {
            throw new HttpRequestException("CreatorApplicationNotFound");
        ...
        var idFilter = Builders<CreatorApplication>.Filter.Eq(
            r => r.Id,
            input.CreatorApplicationId
        );
        var userUpdates = Builders<CreatorApplication>
            .Update.Set(r => r.Status, CreatorApplicationStatus.APPROVED)
        ...
        if (user.PhoneNumber is not null && user.PhoneNumberVerifiedAt is not null)
        {
            var _ = SmsConfiguration.SendSms(
                ...
                {
                    PhoneNumber = user.PhoneNumber,
                    Message = EmailTemplates.CreatorApplicationApprovedBody.Replace(
                        "{name}",
                        user.Name
                    ),
                    AppId = _appConstantsConfiguration.SmsAppId,
    ...
    public async Task<CreatorApplication> RejectCreatorApplication(
        RejectCreatorApplicationInput input,
        string adminId
    )
    {
        var admin = await _adminCollection.Find(admin => admin.Id == adminId).FirstOrDefaultAsync();
        ...
        var userUpdates = Builders<CreatorApplication>
            .Update.Set(r => r.Status, CreatorApplicationStatus.REJECTED)
            ...
            var _ = SmsConfiguration.SendSms(
                new SendSmsInput
                {
                    ...
                    Message = EmailTemplates
                        .CreatorApplicationRejectedBody.Replace("{name}", user.Name)
